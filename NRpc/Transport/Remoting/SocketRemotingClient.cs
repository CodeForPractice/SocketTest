...
using NRpc.Transport.Remoting.Exceptions;
...
namespace NRpc.Transport.Remoting
{
    public class SocketRemotingClient
    {
        ...
        public SocketRemotingClient(EndPoint serverEndPoint, SocketSetting setting = null, EndPoint localEndPoint = null)
        {
            ...
            _scheduleService = ContainerManager.Resolve<IScheduleService>();
        ...
        private void HandleServerPushMessage(ITcpConnection connection, RemotingServerMessage message)
        {
            ...
            {
                LogUtil.ErrorFormat("No handler found for remoting server message:{0}", message);
            }
        }
            ...
            foreach (var key in timeoutKeyList)
            {
                ...
                if (_responseFutureDict.TryRemove(key, out responseFuture))
                {
                    ...
                    responseFuture.SetResponse(new RemotingResponse(
                        ...
                        request.Header,
                        null));
                    LogUtil.DebugFormat("Removed timeout request:{0}", responseFuture.Request);
                }
            }
        }
        ...
        {
            LogUtil.InfoFormat("Try to reconnect to server, address: {0}", _serverEndPoint);
            ...
            {
                LogUtil.Error("Reconnect to server error.", ex);
                ExitReconnecting();
            }
        }
